...
function createPeerToPeer({
  ...
  const sendMessage = (message) => channel.send(message);
...
var datetimeRegex2 = function(args) {
...
var custom2 = function(check, params = {}, fatal) {
  if (check)
...
// src/types.ts
var newGame = z2.object({
  kind: z2.literal("new-game"),
...
// src/p2p.ts
function initPeerToPeer(redraw) {
  return createPeerToPeer({
    ...
    onMessage(rawMessage) {
      const message = messageSchema2.parse(JSON.parse(rawMessage));
      switch (message.kind) {
        case "new-game": {
...
var gameId = function() {
  ...
  if (paramsGameId)
    return paramsGameId;
  const gameId2 = randomId();
  window.history.pushState(null, "", `?game=${gameId2}`);
  return gameId2;
};
